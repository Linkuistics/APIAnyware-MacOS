...
`wkwebview.rkt` references `_NSEdgeInsets` (used in `WKWebView` geometry properties), which is defined and provided by `runtime/type-mapping.rkt`. The generator does not emit a conditional `(require ... type-mapping.rkt)` for `wkwebview.rkt` because `any_struct_type` in `shared_signatures.rs` does not detect `NSEdgeInsets` in WKWebView's property set. Result: anything transitively importing `wkwebview.rkt` cannot load in isolation. **Workaround:** use raw `tell` instead of generated WKWebView bindings. Core backlog task filed 2026-04-16. When fixed, add `wkwebview.rkt` to `LIBRARY_LOAD_CHECKS`.
