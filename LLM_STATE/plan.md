...
- **Geometry struct aliases must be recognized in FFI mapper:** libclang classifies `NSRect`, `CGRect`, `NSPoint`, `CGPoint`, `NSSize`, `CGSize`, `NSRange` as `TypeRefKind::Alias` (typedefs), not `TypeRefKind::Struct`. The FFI mapper must check alias names against a known geometry struct list and map them to `_NSRect`/`_NSPoint`/`_NSSize`/`_NSRange` instead of the default `_uint64`. Without this fix, typed `objc_msgSend` bindings pass 8 bytes instead of 32 for NSRect params, causing crashes. The `is_struct_type` trait method also needs to recognize these aliases. Generated files that use struct types must `(require "type-mapping.rkt")` for the cstruct definitions.
- **Option set enum values not extracted:** NSWindowStyleMask, NSBackingStoreType, NSTextAlignment and similar option-set/bitmask enums have 0 values in the enriched IR. The collector creates the enum entry but doesn't extract the individual flag constants. Sample apps must define these constants inline for now. The POC had them — this is a gap in the new extraction.
- **macOS NSTextAlignment values differ from guesses:** NSTextAlignmentLeft=0, NSTextAlignmentCenter=1, NSTextAlignmentRight=2. Getting these wrong (e.g., using 2 for center) produces visibly wrong layout. Always verify enum constant values against Apple headers or the POC's extracted values, never guess.
